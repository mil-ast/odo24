...
export class MainComponent implements OnInit {
  ...
  ngOnInit() {
    forkJoin(
      ...
      // группируем сервисы
      const services_groups: Map<number, Service[]> = new Map();
      ...
      for (let i = 0; i < groups.length; i++) {
        this.groups.push(new Group(groups[i].group_id, groups[i].name, groups[i].global, services_groups.get(groups[i].group_id)));
      }
